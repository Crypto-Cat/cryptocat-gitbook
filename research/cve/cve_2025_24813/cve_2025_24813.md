...
Depending on preference, we might also like to save the two functions and diff in the terminal. I've added some comments to correlate with the numbered list below:
...
protected boolean checkIfRange(HttpServletRequest request, HttpServletResponse response, WebResource resource) throws IOException {
    ...
    // [0] If no If-Range header present, allow range handling to proceed
    if (!headerEnum.hasMoreElements()) {
        return true;
    }
    String headerValue = headerEnum.nextElement();
    // [1] Reject multiple If-Range headers (ambiguous/malicious); send 400
    if (headerEnum.hasMoreElements()) {
        response.sendError((int) WebdavStatus.SC_BAD_REQUEST);
        return false;
    }
    ...
    // [2] If header contains a date, only allow range when resource last-modified is within 1s
    if (headerValueTime != -1) {
        return resourceLastModified >= headerValueTime && resourceLastModified < headerValueTime + 1000;
    }
    // [3] If header is an ETag: validate quoted syntax; malformed -> 400
...
-    if (this.readOnly) {
+    if (isReadOnly()) { // [0] If the server is not writable, return (this would have stopped original exploit)
...
+    if (range == null || !checkIfHeaders(req, resp, resource)) { // [1] Add header validation (including If-Range changes seen earlier), return if it fails
         return;
     }
     InputStream resourceInputStream2 = null;
+    File tempContentFile = null; // [2] Temp file holder
     try {
         if (range == IGNORE) {
...
+            tempContentFile = executePartialPut(req, range, path); // [2] Write to temp file first
...
+        if (tempContentFile != null) { // [3] Cleanup temp on success
...
+        if (0 != 0) { // [3] Cleanup temp on error path
+            tempContentFile.delete();
+        }
         throw th;
     }
...
-    String convertedResourcePath = path.replace('/', '.'); // [0] Stop replacing forward slashes (`/`) in the path with dots (`.`)
-    File contentFile = new File(tempDir, convertedResourcePath);
-    if (contentFile.createNewFile()) {
-        contentFile.deleteOnExit();
-    }
+    File contentFile = File.createTempFile("put-part-", null, tempDir); // [1] Unpredictable temp filename
